/*
    ...
    Given an array of numbers, write a function that returns a new array of
    ...
    // prints [ 3, 4, 5, 6, 9, 10, 14, 16, 21 ]
*/

function sort(nums, nuArr = []) {
  debugger;
  if (nums.length === 0) return nuArr;
  let largestNum = Math.min(...nums);
  let index = nums.indexOf(largestNum);
  nuArr.push(nums.splice(index, 1)[0]);
  return sort(nums, nuArr);
}
